...
?>
<form id="form1" name="form1" method="post" action="savemtbiginfo.php?action=edit&ID=<?php echo $ID;?>&titleID=<?php echo $titleID;?>&sh=<?php echo $_REQUEST["sh"];?>" onSubmit="return CheckForm();">
  ...
  <table width="98%" align="center" border="0" cellpadding="4" cellspacing="1" bgcolor="#e2e2e2" style="margin-bottom:8px">
    ...
    <tr bgcolor="#FFFFFF">
      ...
      <td><select name="nav_id" size="1" id="nav_id">
          <option value="46" selected>软文发布</option>
...
<?php } ?>
    ...
    <tr bgcolor="#FFFFFF" class='yincang' <?php if($rsN["typefl"]==3 ||$rsN["typefl"]==4) {echo "style='display:none'";}?>>
...
function getVal(){
	...
	$(document).ready(function() {
    $('input[type=radio][name=typefl]').change(function() {
        if (this.value == '3') {
           $(".yincang").hide();
        }
