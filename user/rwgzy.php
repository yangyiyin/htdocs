...
if($_GET["fl"]=="zmt"){
$fileName='软文阁自媒体报价表';
$sql="select SmallClass,medianame,anliurl,price,linkurl,xinwenyuan,content,zhmh from media_info,media_class where zhmh  IN (select BigClass from menhu_class where paixu<50) AND media_class.id=media_info.class_id AND media_info.sh=1 order by medianame desc";	
}else{
$fileName='软文阁优势媒体报价表';
$sql="select SmallClass,medianame,anliurl,price,linkurl,xinwenyuan,content,zhmh from media_info,media_class where zhmh NOT IN (select BigClass from menhu_class where paixu<50) AND media_class.id=media_info.class_id AND media_info.sh=1 order by medianame desc";
