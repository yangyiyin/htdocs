...
else
{
  ...
  {
   case "add":
   $sql="insert into ".$titleID."_info (class_id,title,entitle,meiti,url,images,nd,content,encontent,tj,data) values (".$class_id.",'".$title."','".$entitle."','".$meiti."','".$url."','".$images."',".$nd.",'".$content."','".$encontent."',".$tj.",'".date("Y-m-d")."')";
  ...
  $sql="update ".$titleID."_info set class_id=".$class_id.",title='".$title."',entitle='".$entitle."',meiti='".$meiti."',url='".$url."',images='".$images."',nd=".$nd.",content='".$content."',encontent='".$encontent."',tj=".$tj.",data='".date("Y-m-d")."' where ID=".$ID."";
  //mysql_db_query($dbname,$sql);
  mysql_select_db($dbname);
  $result=mysql_query($sql);
