...
$sql="select * from tixian order by ID desc limit ".($List_Num*($Page-1)).",".$List_Num."";
...
<TABLE cellSpacing=1 cellPadding=0 width="100%" bgColor=#e2e2e2 style="font-size:12px">
  ...
  <TR>
	...
	<TD bgColor=#ffffff>
	  ...
	  <TD bgColor=#ffffff>
      <DIV align=center>账户名称</DIV></TD>
    <TD bgColor=#ffffff>
      <DIV align=center>申请金额（元）</DIV></TD>
    <TD bgColor=#ffffff>
      <DIV align=center>办理状态</DIV></TD>
	<TD bgColor=#ffffff>
      <DIV align=center width="210">管理操作</DIV></TD></TR>
	<?php
		$i=1;
  ...
  <TR>
	...
	if($rs['flag']==0 and $rs['dyn']==0){?>
		<A href="tixianinfo.php?ID=<?php echo $rs["ID"];?>">预览</A> <A href="savetixian.php?ID=<?php echo $rs["ID"];?>&action=isyn" onclick="if(confirm('您确认要办理该提现申请吗？')) return true;else return false; ">办理</A>
	<?php }
	elseif($rs['flag']==0 and $rs['dyn']==1){?>
		<A href="tixianinfo.php?ID=<?php echo $rs["ID"];?>">预览</A>
	<?php }
	elseif($rs['flag']==1){?>
		<A href="tixianinfo.php?ID=<?php echo $rs["ID"];?>">预览</A> <a href="tixian_ok.php?ID=<?php echo $rs["ID"];?>">办结</a>
	<?php }
	else{?>
		<a href="tixianinfo.php?ID=<?php echo $rs["ID"];?>">预览</a>
	<?php } ?>
	  ...
	  <table width="100%" border="0" cellspacing="0" cellpadding="2" style="font-size:12px">
            <tr>
                <td width="87%" height="35" align="center"><DIV class=pager><?php
					  ...
					  {
					  $Prv=$Page-1;
					  echo "<a href='tixianlist.php'>首页</a>&nbsp;&nbsp;";
					  echo "<a href=tixianlist.php?Page=".$Prv.">上一页</a>&nbsp;&nbsp;";
					  }
					  else
					  {
					   echo "首页&nbsp;&nbsp;";
					...
					echo $str;
					  ...
					  $NtPage=$Page+1;
					  $EdPage=$CountPage;
					  echo "<a href=tixianlist.php?Page=".$NtPage.">下一页</a>&nbsp;&nbsp;";
					  echo "<a href=tixianlist.php?Page=".$EdPage.">尾页</a>&nbsp;&nbsp;";
					  }
					  else
					  {
					   echo "下一页&nbsp;&nbsp;";
