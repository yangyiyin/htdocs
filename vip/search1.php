...
if($_GET["fl"]<>""){
	$id=$_GET["fl"];
	$sql="select * from media_info where sh=1 and VipUser='".$VipUser."' and class_id=".$id." order by ID asc";
}
elseif($_GET["lj"]<>""){
	$id=$_GET["lj"];
	$sql="select * from media_info where sh=1 and VipUser='".$VipUser."' and linkurl='".$id."' order by ID asc";
}
elseif($_GET["xw"]<>""){
	$id=$_GET["xw"];
	$sql="select * from media_info where sh=1 and VipUser='".$VipUser."' and xinwenyuan='".$id."' order by ID asc";
}
elseif($_GET["jg"]<>"" and $_GET["jg"]<>"all"){
	...
	switch($_GET["jg"])
		{
		  case "0":
		   $sql="select * from media_info where (scprice between 0 and 40) and sh=1 and VipUser='".$VipUser."' order by ID asc";
		   break;
		 case "1":
		   $sql="select * from media_info where (scprice between 41 and 70) and sh=1 and VipUser='".$VipUser."' order by ID asc";
		   break;
		 case "2":
		   $sql="select * from media_info where (scprice between 71 and 120) and sh=1 and VipUser='".$VipUser."' order by ID asc";
		   break;
		 case "3":
		   $sql="select * from media_info where (scprice >120) and sh=1 and VipUser='".$VipUser."' order by ID asc";
		   break;
		 default:
		   $sql="select * from media_info where sh=1 and VipUser='".$VipUser."' order by ID asc";
		}
}
elseif($_GET["dq"]<>""){
	$id=$_GET["dq"];
	$sql="select * from media_info where sh=1 and VipUser='".$VipUser."' and diqu='".$id."' order by ID asc";
}
elseif($_GET["zhmh"]<>""){
	$id=$_GET["zhmh"];
	$sql="select * from media_info where sh=1 and VipUser='".$VipUser."' and zhmh='".$id."' order by ID asc";
}
else{
	$mc=$_GET["mc"];
	$cid=$_GET["cid"];
