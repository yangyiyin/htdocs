...
elseif($_GET["jg"]<>"" and $_GET["jg"]<>"all"){
	...
	switch($_GET["jg"])
		{
		 case "0":
		   $sql="select * from media_info where sh=1 and (scprice between 0 and 40) order by ID asc";
		   break;
		 case "1":
		   $sql="select * from media_info where sh=1 and (scprice between 41 and 70) order by ID asc";
		   break;
		 case "2":
		   $sql="select * from media_info where sh=1 and (scprice between 71 and 120) order by ID asc";
		   break;
		 case "3":
		   $sql="select * from media_info where sh=1 and scprice >120 order by ID asc";
		   break;
		 default:
...
<?php
    ...
    <td width="130" ><div align="center"><a href='../mtinfo.php?ID=<?php echo $rs["ID"];?>' target="_blank"><?php echo $rs["medianame"];?></a><span style="float:right"><a href='<?php echo $rs["anliurl"];?>' target="_blank"><img  alt="" src="<?php if ($rs["VipUser"]<>""){echo '../images/qiuqiu1.png';} else{echo '../images/qiuqiu2.png';}?>" name="qiu"  height="12"/></a></span></div></td>
