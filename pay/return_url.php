...
if($verify_result) {//验证成功
	...
	if($rss!=NULL)
	{
		$sql="insert into caiwu (order_id,trade_id,VipUser,operator,leixing,jine,flag,data) values ('".$order_id."','".$trade_id."','".$VipUser."','".$VipUser."','".$leixing."',".$jine.",".$flag.",'".$data."')";
		mysql_db_query($dbname,$sql);

		$yue=$rss["yue"]+$jine;
		$sqlu="update user set yue=".$yue." where VipUser='".$VipUser."'";
		mysql_db_query($dbname,$sqlu);
		echo "<script>alert('充值成功！');location.href='../user/index.php';</script>";
	}
	else{
		echo "<script>alert('充值异常，请与站长联系！');location.href='../user/index.php';</script>";
	}
